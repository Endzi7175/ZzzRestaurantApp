...
export class RestaurantTableComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.restaurantTableService
        ...
        .subscribe({
          next: (res: HttpResponse<IRestaurantTable[]>) => {
  ...
  trackId(index: number, item: IRestaurantTable): number {
    return item.id!;
  }

  delete(restaurantTable: IRestaurantTable): void {
    const modalRef = this.modalService.open(RestaurantTableDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IRestaurantTable[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
